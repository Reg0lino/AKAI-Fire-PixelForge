...
class MonitorViewWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        # These store the current state of the selection box in LOGICAL pixels of the ORIGINAL target monitor
        # They are primarily updated by user interaction OR by loading saved percentages.
    ...
    # --- Helper Methods (Snapping, Clamping, Handles) ---
    def _get_handle_rects(self) -> dict[ResizeHandle, QRectF]: # Unchanged
    ...
    def _get_handle_at_pos(self, pos: QPointF) -> ResizeHandle: # Unchanged
    ...
    def _clamp_selection_rect_to_current_monitor_view(self): # Unchanged
    ...
    def _get_snap_points_scaled(self) -> dict[str, QPointF]: # Unchanged
        points = {}; 
        ...
        return points
    def _try_snap_selection_drag(self): # Unchanged
        if self.selection_rect_scaled.isNull(): return
        ...
        if final_offset_to_apply.x() != 0 or final_offset_to_apply.y() != 0:
            ...
            self._clamp_selection_rect_to_current_monitor_view()
    def _try_snap_selection_resize(self): # Unchanged
    ...
    # --- Public Accessors (Example) ---
    def get_current_selection_parameters(self) -> tuple[int | None, dict | None]:
        # This should now also derive percentages from the internal logical state
        if self.target_monitor_mss_id is None: return None, None
        # Create a temporary region_data dict by calling _emit_selection_changed's logic
        # This is a bit of a workaround; ideally _emit_selection_changed doesn't have side effects
        # For now, let's replicate its core calculation
        target_mon_info = next((m for m in self.all_monitors_original_info if m['id'] == self.target_monitor_mss_id), None)
...
if __name__ == '__main__':
    ...
    # Let's test setting params explicitly after initial setup
    # QTimer.singleShot(100, lambda: monitor_view_main.set_current_selection_from_params(1, {'x':0.1, 'y':0.1, 'width':0.3, 'height':0.15}))
