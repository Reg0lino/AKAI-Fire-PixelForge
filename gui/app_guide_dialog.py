...
<div class="container">
    ...
    <h3>Animator Timeline & File Management</h3>
    <p>The timeline is your main workspace for building animations. You can select, multi-select (with <code>Ctrl</code> or <code>Shift</code>), and drag-and-drop frames to reorder them. Use the buttons or hotkeys for operations like adding, duplicating, and deleting frames. The dropdown menu allows you to browse and load your saved sequences.</p>
    <h3>📥 GIF Importer</h3>
    <p>Quickly create complex animations by importing existing GIFs directly into the Animator Studio. Click the <code>GIF Import</code> button to open the importer dialog.</p>
    <ul>
        <li><b>Source:</b> Load a GIF from a local file or a direct URL.</li>
        <li><b>Region Selection:</b> Click and drag on the main preview image to define a specific area of the GIF to capture. Click the "Select Full GIF" button to reset to the full area.</li>
        <li><b>Live Previews:</b> The "Live Pad Preview" shows a real-time, 1:1 representation of what the final animation will look like on the hardware pads. This preview will also be mirrored live on the physical device if it is connected.</li>
        <li><b>Play Original:</b> Click "▶️ Play Original GIF" to view the source GIF in a separate window, helping you decide which part to sample.</li>
        <li><b>Color & Speed Controls:</b> Adjust the brightness, saturation, contrast, and hue of the imported frames. You can also override the GIF's original speed with a custom FPS setting.</li>
    </ul>
    ...
    <p>Dynamically mirror colors from your screen onto the pads for ambient lighting or to record new animations.</p>
    <ul>
        ...
        <li><b>Mode Selection:</b> Use the "Mode" dropdown in the UI or the <span class="key-cap">NOTE</span> button on the hardware to cycle through different sampling algorithms (Region, Thumbnail, Palette).</li>
        ...
        <li><b>Record Output:</b> Click "Record" to capture the sampler's visual output directly into a new animation sequence.</li>
    </ul>
...
class AppGuideDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(
            "🚀 Akai Fire PixelForge - App Guide & Hotkeys (v2.0.0)")
    ...
    def _create_guide_tab(self) -> QWidget:
        ...
        master_scroll_area.setObjectName("GuideScrollArea")
        master_scroll_area.setFrameShape(QFrame.Shape.NoFrame)
        guide_container = QWidget()
        guide_container.setObjectName("GuideTabContainer")
    ...
    def _create_controls_tab(self) -> QWidget:
        ...
        hw_buttons = [
            ...
            ("STEP", "Toggle Audio Visualizer ON/OFF."),
            ("ALT", "Toggle the 'Enable Color FX' checkbox."),
    ...
    def _apply_styling(self):
        self.setStyleSheet("""
            ...
            QWidget#GuideTabContainer { background-color: #000000; }
            QTabWidget::pane { border: 1px solid #161616; background-color: #000000; }
            ...
            QTabBar::tab:selected { background-color: #000000; color: #6366f1; margin-bottom: -1px; }
            QScrollArea { border: none; }
            QGroupBox { font-family: 'Press Start 2P'; font-size: 11pt; color: #22d3ee; background-color: #000000; border: 1px solid #000000; border-radius: 6px; margin-top: 10px; padding: 20px 15px 15px 15px; }
            ...
            QScrollArea QScrollBar:vertical { border: none; background: #000000; width: 12px; margin: 0px 0px 0px 0px; }
            QScrollArea QScrollBar::handle:vertical { background: #1a1a2e; min-height: 20px; border-radius: 6px; }
            QScrollArea QScrollBar::handle:vertical:hover { background: #6366f1; }
            QScrollArea QScrollBar::add-line:vertical, QScrollArea QScrollBar::sub-line:vertical { border: none; background: none; height: 0px; }
            QScrollArea QScrollBar::add-page:vertical, QScrollArea QScrollBar::sub-page:vertical { background: none; }
        """)
