...
class VisualizerSettingsDialog(QDialog):
    ...
    def __init__(self, current_mode_key: str, all_current_settings: dict | None, config_save_path_func, parent=None):
        ...
        # This will be the dialog's working copy of settings (UI Scale)
        self.all_settings = {}

        # Deep copy initial settings to self.all_settings
        if isinstance(all_current_settings, dict):
            for mode_key_iter, settings_val in all_current_settings.items():
                if isinstance(settings_val, dict):
                    ...
                    # Ensure lists within are also copied
                    # Add any other list-type settings
                    for list_key in ["band_colors", "palette", "spectrum_colors"]:
        ...
        else:  # Fallback if no settings passed or invalid
            self.all_settings = {}

        # Ensure default structures exist for all modes in self.all_settings (UI Scale)
        csb_defaults_ui = {
        ...
        self.all_settings.setdefault("classic_spectrum_bars", {}).update(
            {k: self.all_settings.get("classic_spectrum_bars", {}).get(
                k, v) for k, v in csb_defaults_ui.items()}
        )
        # Ensure band_colors is a list copy
        self.all_settings["classic_spectrum_bars"]["band_colors"] = list(
            self.all_settings["classic_spectrum_bars"]["band_colors"])

        pw_defaults_ui = {
            "color": QColor("cyan").name(),
            "speed": 50,  # UI scale 0-100
            "brightness_sensitivity": 75  # UI scale 0-100
        }
        self.all_settings.setdefault("pulse_wave_matrix", {}).update(
            {k: self.all_settings.get("pulse_wave_matrix", {}).get(
                k, v) for k, v in pw_defaults_ui.items()}
        )
        # Ensure sliders are integers for UI
        self.all_settings["pulse_wave_matrix"]["speed"] = int(
            self.all_settings["pulse_wave_matrix"]["speed"])
        self.all_settings["pulse_wave_matrix"]["brightness_sensitivity"] = int(
            self.all_settings["pulse_wave_matrix"]["brightness_sensitivity"])
        ...
        dvu_defaults_ui = {
            ...
            # Central spectrum settings for this mode will be added here later
        }
        self.all_settings.setdefault("dual_vu_spectrum", {}).update(
            {k: self.all_settings.get("dual_vu_spectrum", {}).get(
                k, v) for k, v in dvu_defaults_ui.items()}
        )
        # Ensure sliders are integers for UI
        for key_dvu_slider in ["vu_threshold_mid", "vu_threshold_high", "vu_falloff_speed"]:
            self.all_settings["dual_vu_spectrum"][key_dvu_slider] = int(
                self.all_settings["dual_vu_spectrum"][key_dvu_slider])
        ...
        # Palette management for Spectrum Bars
        ...
        # --- End Dual VU UI elements ---

        # Stores user-saved profiles (name -> {mode_key -> settings_dict})
        self.color_profiles = {}

        self._init_ui()  # Creates tab structure and assigns UI elements to self
        self._load_color_profiles()  # Load all user profiles
        self._populate_ui_from_settings()  # Populate all tabs based on self.all_settings
        self._select_initial_tab()  # Select the tab that was active when dialog was opened
    ...
    def _collect_settings_from_all_tabs(self) -> dict:
        ...
        # Add central spectrum settings collection here later
    ...
    # Placeholder for methods that were part of the old QListWidget system, no longer directly used for Spectrum Bars
    ...
    def _create_dual_vu_settings_tab(self) -> QWidget:
        ...
        description_label = QLabel(
            "Configure two VU meters on the edges and a central spectrum display. Customize colors and behaviors for each.")
        description_label.setWordWrap(True)
        page_layout.addWidget(description_label)
        page_layout.addWidget(
            QFrame(frameShape=QFrame.Shape.HLine, frameShadow=QFrame.Shadow.Sunken))
        ...
        vu_settings_layout.addWidget(
            QLabel("Low Color:"), 0, 0, Qt.AlignmentFlag.AlignRight)
        self.dvu_low_color_button = ColorGradientButton(parent=self)
        self.dvu_low_color_button.clicked.connect(
            lambda: self._on_dvu_color_changed("low"))
        vu_settings_layout.addWidget(self.dvu_low_color_button, 0, 1)
        vu_settings_layout.addWidget(
            QLabel("Mid Color:"), 1, 0, Qt.AlignmentFlag.AlignRight)
        self.dvu_mid_color_button = ColorGradientButton(parent=self)
        self.dvu_mid_color_button.clicked.connect(
            lambda: self._on_dvu_color_changed("mid"))
        vu_settings_layout.addWidget(self.dvu_mid_color_button, 1, 1)
        vu_settings_layout.addWidget(
            QLabel("High Color:"), 2, 0, Qt.AlignmentFlag.AlignRight)
        self.dvu_high_color_button = ColorGradientButton(parent=self)
        self.dvu_high_color_button.clicked.connect(
            lambda: self._on_dvu_color_changed("high"))
        vu_settings_layout.addWidget(self.dvu_high_color_button, 2, 1)
        vu_settings_layout.addItem(QSpacerItem(
            20, 10, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Fixed), 3, 0, 1, 3)
        vu_settings_layout.addWidget(
            QLabel("Mid Threshold (%):"), 4, 0, Qt.AlignmentFlag.AlignRight)
        self.dvu_threshold_mid_slider = QSlider(Qt.Orientation.Horizontal)
        self.dvu_threshold_mid_slider.setRange(0, 100)
        self.dvu_threshold_mid_slider.valueChanged.connect(
            self._on_dvu_threshold_mid_changed)
        ...
        vu_settings_layout.addWidget(
            QLabel("High Threshold (%):"), 5, 0, Qt.AlignmentFlag.AlignRight)
        self.dvu_threshold_high_slider = QSlider(Qt.Orientation.Horizontal)
        self.dvu_threshold_high_slider.setRange(0, 100)
        self.dvu_threshold_high_slider.valueChanged.connect(
            self._on_dvu_threshold_high_changed)
        ...
        vu_settings_layout.addWidget(
            QLabel("Falloff Speed:"), 6, 0, Qt.AlignmentFlag.AlignRight)
        self.dvu_falloff_speed_slider = QSlider(Qt.Orientation.Horizontal)
        self.dvu_falloff_speed_slider.setRange(0, 100)
        self.dvu_falloff_speed_slider.valueChanged.connect(
            self._on_dvu_falloff_speed_changed)
        ...
        spectrum_layout.addWidget(
            QLabel("Controls for the central spectrum will appear here."))
        page_layout.addWidget(spectrum_group)

        palette_group_dvu = QGroupBox(
            "Scheme Palette Management (Coming Soon)")
        palette_layout_dvu = QVBoxLayout(palette_group_dvu)
        palette_layout_dvu.addWidget(QLabel(
            "Profile saving/loading for Dual VU & Spectrum schemes will appear here."))
        page_layout.addWidget(palette_group_dvu)
